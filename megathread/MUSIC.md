![](%%music%%)

# 🎹 ➜ [Megathread](https://www.reddit.com/r/Piracy/wiki/megathread/) / **Music**

### ❕ ➜ Quick reminder
- ✔️ All links have been reviewed and approved by moderators, but proceed with caution.
- 👍🏻 Rather having an enormous variety, our [Megathread](https://www.reddit.com/r/Piracy/wiki/megathread/) provides only the finest options.
- 🐐 [GOAT](https://www.urbandictionary.com/define.php?term=goat) status = Highly regarded by the pirate community.

&nbsp;

---

&nbsp;

# 📑 ➜ Browser Downloaders

### 🌐 [ArtistGrid](https://artistgrid.cx/)
- Download unreleased music.
- [URL Safety Results](https://www.urlvoid.com/scan/artistgrid.cx/)

### 🌐 [AZMP3](https://azmp3.cc/)
- YouTube Converter that transforms your favorite videos into high-quality MP3 audio.
- [URL Safety Results](https://www.urlvoid.com/scan/azmp3.cc/)

### 🌐 [BestMP3Converter](https://www.bestmp3converter.com/)
- Enables you to obtain audio files from YouTube videos without charge in 320kbps.
- [URL Safety Results](https://www.urlvoid.com/scan/bestmp3converter.com/)

### 🌐 [CnvMP3](https://cnvmp3.com/)
- Ad-free YouTube downloads as MP3 or MP4 up to 320kbps.
- [URL Safety Results](https://www.urlvoid.com/scan/cnvmp3.com/)

### 🐐 [cobalt](https://cobalt.tools/) / [2](https://instances.cobalt.best/)
- Preserve music and videos you hold dear, free from intrusive advertisements.
- [URL Safety Results](https://www.urlvoid.com/scan/cobalt.tools/)

### 🌐 [DeezMate](https://deezmate.com/)
- Clean, user-friendly site for downloading Deezer tracks as MP3 or FLAC.
- [URL Safety Results](https://www.urlvoid.com/scan/deezmate.com/)

### 🐐 [DoubleDouble](https://doubledouble.top/)
- Music downloads through Spotify, Apple or Amazon Music, Soundcloud, Qobuz, Deezer, Tidal, Napster, Beatport or KKBOX.
- [URL Safety Results](https://www.urlvoid.com/scan/doubledouble.top/)

### 🌐 [Download from Bandcamp](https://downloadmusicschool.com/bandcamp/)
- Ssearch a Bandcamp page for music streams and save them as MP3 files.
- [URL Safety Results](https://www.urlvoid.com/scan/downloadmusicschool.com/)

### 🌐 [downloadsound.cloud](https://downloadsound.cloud/)
- Web app project built for downloading SoundCloud tracks and playlists.
- [URL Safety Results](https://www.urlvoid.com/scan/downloadsound.cloud/)

### 🌐 [EzMP3](https://ezmp3.cc/)
- Conversion of YouTube videos to MP3, supported by user donations, ensuring a clean UI.
- [URL Safety Results](https://www.urlvoid.com/scan/ezmp3.cc/)

### 🌐 [Free MP3](https://en2.freemp3.tube/)
- Quick downloads, with a search feature to locate desired music from its extensive database.
- [URL Safety Results](https://www.urlvoid.com/scan/en2.freemp3.tube/)

### 🌐 [iViGo](https://ivigo.cc/)
- This downloader can save streaming audio or video in formats like .mp3 and .mp4.
- [URL Safety Results](https://www.urlvoid.com/scan/ivigo.cc/)

### 🌐 [Locoloader](https://www.locoloader.com/)
- Save videos, music, and photos from well-known websites, on any device.
- [URL Safety Results](https://www.urlvoid.com/scan/locoloader.com/)

### 🐐 [lucida](https://lucida.to/)
- High-quality music at no cost from platforms like SoundCloud, Deezer, Spotify, Qobuz, and Tidal.
- [URL Safety Results](https://www.urlvoid.com/scan/lucida.to/)

### 🌐 [MonoChrome](https://monochrome.tf/)
- Free music streaming/downloading from Tidal.
- [URL Safety Results](https://www.urlvoid.com/scan/monochrome.tf/)

### 🌐 [MYFREEMP3](https://2024.myfreemp3juices.cc/)
- Rapidly access song downloads, featuring a search tool to pinpoint your favorite tunes.
- [URL Safety Results](https://www.urlvoid.com/scan/2024.myfreemp3juices.cc/)

### 🌐 [OFA Downloader](https://oneforalldownloader.com/)
- Great web-based tool to download videos from YouTube, TikTok, and other sites.
- [URL Safety Results](https://www.urlvoid.com/scan/oneforalldownloader.com/)

### 🌐 [Racoon](https://shailendramaurya.github.io/racoon/)
- Enjoy high-quality downloads of YouTube videos and audio for free, with no ads.
- [URL Safety Results](https://www.urlvoid.com/scan/shailendramaurya.github.io/)

### 🌐 [SpotiDownloader](https://spotidownloader.com/)
- Grab any Spotify track, album, or playlist in seconds with this free, easy-to-use downloader.
- [URL Safety Results](https://www.urlvoid.com/scan/spotidownloader.com/)

### 🌐 [SpotifyMate](https://spotifymate.com/)
- Nice online tool to download Spotify songs, playlists, and albums in mp3 format.
- [URL Safety Results](https://www.urlvoid.com/scan/spotifymate.com/)

### 🌐 [SoundCloud to MP3](https://soundcloudmp3.cc/)
- Download a wide array of songs from SoundCloud for your listening enjoyment.
- [URL Safety Results](https://www.urlvoid.com/scan/soundcloudmp3.cc/)

### 🐐 [SQUID.WTF](https://squid.wtf/)
- Provides an accessible GUI interface for the Deezer and Qobuz library.
- [URL Safety Results](https://www.urlvoid.com/scan/squid.wtf/)

### 🌐 [YTiz](https://ytiz.xyz/)
- Songs from YouTube, SoundCloud, BandCamp, Twitter, and TikTok, with no ads or trackers.
- [URL Safety Results](https://www.urlvoid.com/scan/ytiz.xyz/)

### 🌐 [Ytmdl Web](https://ytmdl.deepjyoti30.dev/) / [2](https://github.com/deepjyoti30/ytmdl-web-v2)
- Get songs from various sources, including iTunes, Gaana, LastFM, and more, with metadata.
- [URL Safety Results](https://www.urlvoid.com/scan/ytmdl.deepjyoti30.dev/)

### 🌐 [YTMP3Hub](https://ytmp3hub.com/)
- Download free MP3 files from YouTube, which is quick and really simple to use. 
- [URL Safety Results](https://www.urlvoid.com/scan/ytmp3hub.com/)

&nbsp;

---

&nbsp;

# 📑 ➜ Desktop Downloaders

### 🖥️ [BlockTheSpot](https://github.com/mrpond/BlockTheSpot)
- All-in-one Spotify skip-bypass and adblocker, offering premium features without downloads.
- [URL Safety Results](https://www.urlvoid.com/scan/github.com/)

### 🖥️ [Downline](https://stefnotch.github.io/downline/) / [2](https://github.com/stefnotch/downline)
- Cross-platform, serving as a frontend for [YT-DLP](https://github.com/yt-dlp/yt-dlp), enabling downloads from YouTube and other sites.
- [URL Safety Results](https://www.urlvoid.com/scan/stefnotch.github.io/)

### 🖥️ [media-downloader](https://github.com/BioHapHazard/Freedify)
- Provides a customizable list of preset options for downloading media, allowing ease of access.
- [URL Safety Results](https://www.urlvoid.com/scan/github.com/)

### 🖥️ [Freedify](https://github.com/mhogomchungu/media-downloader/)
- Stream music and podcasts from anywhere. 
- [URL Safety Results](https://www.urlvoid.com/scan/github.com/)


### 🖥️ [muffon](https://muffon.netlify.app/)
- Streamlined music app with seamless listening, exploration, and enhanced organization features.
- [URL Safety Results](https://www.urlvoid.com/scan/muffon.netlify.app/)

### 🖥️ [Nicotine+](https://nicotine-plus.org/)
- Updated GUI for [Soulseek](https://www.slsknet.org/news/), maintaining compatibility with new features.
- [URL Safety Results](https://www.urlvoid.com/scan/nicotine-plus.org/)

### 🖥️ [OnTheSpot](https://github.com/justin025/onthespot)
- Open-source music downloader built with Qt and Python that enables users to search and download songs.
- [URL Safety Results](https://www.urlvoid.com/scan/github.com/)

### 🖥️ [Open Video Downloader](https://github.com/StefanLobbenmeier/youtube-dl-gui)
- Electron and Node.js GUI for [YT-DLP](https://github.com/yt-dlp/yt-dlp), enabling downloads from various platforms.
- [URL Safety Results](https://www.urlvoid.com/scan/github.com)

### 🖥️ [OrpheusDL](https://github.com/yarrm80s/orpheusdl)
- Python-based music archive solution, supporting Deezer, Qobuz, Tidal, and more providers.
- [URL Safety Results](https://www.urlvoid.com/scan/github.com/)

### 🖥️ [Qobuz-DL](https://github.com/vitiko98/qobuz-dl)
- Find, listen to, and download Lossless music from Qobuz. Also supports downloads from Spotify and Apple Music.
- [URL Safety Results](https://www.urlvoid.com/scan/github.com/)

### 🖥️ [SCDL](https://github.com/flyingrub/scdl)
- Compatible across Windows, MacOS, and Linux, download music from SoundCloud and add ID3 tags seamlessly.
- [URL Safety Results](https://www.urlvoid.com/scan/github.com/)

### 🐐 [Soulseek](https://www.slsknet.org/news/)
- Music-sharing client connects you to a P2P network, facilitating the discovery of hard-to-find songs.
- [URL Safety Results](https://www.urlvoid.com/scan/slsknet.org/)

### 🐐 [Spicetify](https://spicetify.app/) / [2](https://github.com/spicetify/cli)
- Command-line utility compatible with Windows, macOS, and Linux for personalizing the official Spotify client.
- [URL Safety Results](https://www.urlvoid.com/scan/spicetify.app/)

### 🖥️ [spotDL](https://spotdl.readthedocs.io/en/latest/)
- Identifies songs from Spotify playlists on YouTube, downloads them, and adds album cover, lyrics, and metadata.
- [URL Safety Results](https://www.urlvoid.com/scan/spotdl.readthedocs.io/)

### 🐐 [Spotube](https://spotube.krtirtho.dev/)
- For every platform, a quick, cutting-edge, lightweight, and effective Spotify Music Client.
- [URL Safety Results](https://www.urlvoid.com/scan/spotube.krtirtho.dev/)

### 🖥️ [SpotX](https://github.com/SpotX-Official/SpotX)
- Customized version of the official Spotify client. Blocks all banner, video, and audio ads and more.
- [URL Safety Results](https://www.urlvoid.com/scan/github.com/)

### 🐐 [Stacher](https://stacher.io/)
- Nice looking GUI frontend for [YT-DLP](https://github.com/yt-dlp/yt-dlp), simplifying audio / video downloads.
- [URL Safety Results](https://www.urlvoid.com/scan/stacher.io/)

### 🖥️ [streamrip](https://github.com/nathom/streamrip)
- A script for a music downloader that works with several platforms, including all the most well-liked ones.
- [URL Safety Results](https://www.urlvoid.com/scan/github.com/)

### 🖥️ [Tartube](https://github.com/axcore/tartube/)
- Streamline your video watching and downloading experience with compatibility across many platforms.
- [URL Safety Results](https://www.urlvoid.com/scan/github.com/)

### 🖥️ [Tidal-Media-Downloader](https://doc.yaronzz.com/post/tidal_dl_installation/)
- Download music and movies from Tidal with metadata and choose the video resolution and audio quality.
- [URL Safety Results](https://www.urlvoid.com/scan/doc.yaronzz.com)

### 🖥️ [yt-dlg](https://oleksis.github.io/youtube-dl-gui/) / [2](https://github.com/oleksis/youtube-dl-gui)
- Simple wxPython¹ GUI for [YT-DLP](https://github.com/yt-dlp/yt-dlp), letting you easily download videos and audio.
- [URL Safety Results](https://www.urlvoid.com/scan/oleksis.github.io/)

### 🖥️ [yt-dlp-gui](https://github.com/dsymbol/yt-dlp-gui)
- Visual interface for the [YT-DLP](https://github.com/yt-dlp/yt-dlp), featuring customizable options.
- [URL Safety Results](https://www.urlvoid.com/scan/github.com/)

### 🐐 [YT-DLP](https://github.com/yt-dlp/yt-dlp)
- Get music and videos from more than 1500 websites, such as YouTube, SoundCloud, and more.
- [URL Safety Results](https://www.urlvoid.com/scan/github.com/)

### 🖥️ [ytdlp-interface](https://github.com/ErrorFlynn/ytdlp-interface)
- Windows GUI for [YT-DLP](https://github.com/yt-dlp/yt-dlp), now accepts non-YouTube URLs broadening its capabilities.
- [URL Safety Results](https://www.urlvoid.com/update/github.com/)

### 🖥️ [ytDownloader](https://ytdn.netlify.app/) / [2](https://github.com/aandrew-me/ytDownloader)
- Extract videos and audio from platforms like Youtube, Facebook, Tiktok, Twitch etc.
- [URL Safety Results](https://www.urlvoid.com/update/ytdn.netlify.app/)

&nbsp;

---

&nbsp;

# 📑 ➜ Direct Downloads

### 🔗 [3ost](https://3ost.ru/) • Sign Up • Russian
- Lossless and lossy soundtracks from games, movies, and cartoons are included in this collection.
- [URL Safety Results](https://www.urlvoid.com/scan/3ost.ru/)

### 🔗 [AnimeThemes](https://animethemes.moe/)
- Impressive and reliable collection of anime OPs and EDs themes with easy links.
- [URL Safety Results](https://www.urlvoid.com/scan/animethemes.moe/)

### 🔗 [AniTousen](https://anitousen.com/) • Russian
- Nice musical archive with theme songs from animated works of Japan China, and Korean.
- [URL Safety Results](https://www.urlvoid.com/scan/anitousen.com/)

### 🔗 [CannaPower!](https://canna-power.to/) • German
- MP3 collection offers diverse music genres, with upbeat tracks and melodic ballads.
- [URL Safety Results](https://www.urlvoid.com/scan/canna-power.to/)

### 🔗 [DAB Music Player](https://dabmusic.xyz/) / [2](https://dab.yeet.su/)
- Stream, search, and download music for an exceptional listening experience.
- [URL Safety Results](https://www.urlvoid.com/scan/dabmusic.xyz/)

### 🔗 [Discografiascompletas](https://www.discografiascompletas.cc/) • Spanish
- Access full discographies of CDs and records in high-quality digital formats.
- [URL Safety Results](https://www.urlvoid.com/scan/discografiascompletas.cc/)

### 🔗 [Download Soundtracks](https://download-soundtracks.com/)
- Explore the collection of movie, game, and series soundtracks, featuring all releases.
- [URL Safety Results](https://www.urlvoid.com/scan/download-soundtracks.com/)

### 🔗 [Folksoundomy: Game Soundtracks](https://archive.org/details/folksoundomy_gamesoundtracks)
- Video game music and some anime soundtracks that was created by volunteers.
- [URL Safety Results](https://www.urlvoid.com/scan/archive.org/)

### 🔗 [JPop Singles](https://www.jpopsingles.eu/)
- Music enthusiasts sharing a large collection of amazing Japanese pop music available.
- [URL Safety Results](https://www.urlvoid.com/scan/jpopsingles.eu/)

### 🔗 [Kdramaost](https://kdramaost.com/)
- Enjoy OSTs from Korean, Japanese, Chinese, Thai, and Taiwanese dramas.
- [URL Safety Results](https://www.urlvoid.com/scan/kdramaost.com/)

### 🔗 [KHInsider: Video Game Music](https://downloads.khinsider.com/)
- Excellent site for MP3 and lossless downloads of console and PC game soundtracks.
- [URL Safety Results](https://www.urlvoid.com/scan/downloads.khinsider.com/)

### 🔗 [Kojima48](https://kojima48.blogspot.com/)
- Collection of Japanese pop music shared by passionate music lovers.
- [URL Safety Results](https://www.urlvoid.com/scan/kojima48.blogspot.com/)

### 🔗 [LightAudio](https://web.ligaudio.ru/) • Russian
- Discover and access music from various sources to effortlessly download it in the highest quality.
- [URL Safety Results](https://www.urlvoid.com/scan/web.ligaudio.ru/)

### 🔗 [MikuDB](https://mikudb.moe/)
- World of Vocaloid with an extensive collection from Hatsune Miku, Kagamine Rin, IA, GUMI etc.
- [URL Safety Results](https://www.urlvoid.com/scan/mikudb.moe/)

### 🔗 [Mp3Mango](https://mp3mango.net/) • Russian
- Tap into tracks from all over and snag them in crisp, crystal-clear audio.
- [URL Safety Results](https://www.urlvoid.com/scan/mp3mango.net/)

### 🔗 [MP3Vanced](https://mp3vanced.online/)
- Enjoy your Spotify music offline and ad-free by converting and saving your favorite tracks.
- [URL Safety Results](https://www.urlvoid.com/scan/mp3vanced.online/)

### 🔗 [Renovatio Records](https://renovatiorecords.blogspot.com/)
- Committed in offering extraordinary film music albums for an easy download process.
- [URL Safety Results](https://www.urlvoid.com/scan/renovatiorecords.blogspot.com/)

### 🔗 [Sitting on Clouds](https://www.sittingonclouds.net/)
- Superb soundtrack collection with a heavy emphasis on video games and Japanese anime.
- [URL Safety Results](https://www.urlvoid.com/scan/sittingonclouds.net/)

### 🔗 [SquidBoard](https://www.squid-board.org/) • Sign Up
- Nice forum focused on finding any media soundtrack you desire or require.
- [URL Safety Results](https://www.urlvoid.com/scan/squid-board.org/)

### 🔗 [Sukidesuost](https://sukidesuost.info/)
- Wonderful and trustworthy site to acquire Asian music, mostly utilizing the Mega file host.
- [URL Safety Results](https://www.urlvoid.com/scan/sukidesuost.info/)

&nbsp;

---

&nbsp;

# 📑 ➜ Knowledge & Tokens

### 🐐 [Firehawk52](https://rentry.co/firehawk52)
- Guides on downloading high-quality music files for free with specific tools.
- [URL Safety Results](https://www.urlvoid.com/scan/rentry.co/)

### 📒 [VGMdb](https://vgmdb.net/)
- Comprehensive details on tracklists, and artists for video and anime OSTs.
- [URL Safety Results](https://www.urlvoid.com/scan/vgmdb.net/)

### 📒 [WhereDownloadMusic](https://rentry.co/wheredownloadmusic/)
- Comprehensive details on tracklists, and artists for video and anime OSTs.
- [URL Safety Results](https://www.urlvoid.com/scan/rentry.co/wheredownloadmusic/)

&nbsp;

---

&nbsp;

# 📑 ➜ Streaming

### ▶️ [KissVK](https://kissvk.pro/) • Russian UI
- Stream and download music without installing software, providing a fast and simple method.
- [URL Safety Results](https://www.urlvoid.com/scan/kissvk.pro/)

### ▶️ [MadnessBeat](https://madnessbeat.club/) • Sign Up • Spanish UI
- Listen and download but also follow your favorite artists and craft playlists to enhance your experience.
- [URL Safety Results](https://www.urlvoid.com/scan/madnessbeat.club/)

### ▶️ [MusicDex](https://musicdex.org/)
- Compilation of anime soundtracks, meticulously curated to highlight both openings and endings.
- [URL Safety Results](https://www.urlvoid.com/scan/musicdex.org/)

### ▶️ [Squidify](https://www.squidify.org/)
- Assortment of anime, video game, movie and TV soundtracks for your listening pleasure.
- [URL Safety Results](https://www.urlvoid.com/scan/squidify.org/)

&nbsp;

---

&nbsp;

📜 ❮ Return to [**Megathread**](https://www.reddit.com/r/Piracy/wiki/megathread/)
---